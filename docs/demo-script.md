...
## Sequência (ambiente local, `poetry install --with dev` já executado)

1. `poetry run poe serve` (outro terminal) — sobe a API em `localhost:8000`.
...
3. `poetry run poe demo` (outro terminal) — sobe o Streamlit em `localhost:8501`.
...
7. (Opcional, se o tempo permitir) `poetry run bandit-cli policy-status` para mostrar
...
| Falha | Ação |
|---|---|
| Streamlit não sobe a tempo | Seguir só com a CLI (`poetry run bandit-cli decide`), que não depende de nenhum servidor além de si mesma |
...
| Terminal/ambiente quebrado no dia | Ter um terminal de backup com `poetry run poe serve`/`poetry run poe demo` já testados antes da apresentação |
